        ...
        <div class="px-3 pt-2">
            <main class="form-signin m-auto text-center w-50 p-3 rounded-3 bg-white mt-5">
                <form method="post" novalidate="">
                    <h2 class="h2 mb-3 fw-normal mb-4 mt-2">Search By Home Number</h2>

                    <div class="form-floating mx-4 mb-3">
                        <input type="text" name="homenumber" class="form-control" id="homenumber" placeholder="homenumber">
...
<?php
    ...
    function search($root){  
        ...
        search($root->left);
        $GLOBALS["i"]++;
        $GLOBALS["searchhomenumber"][$GLOBALS["i"]] = $root->value;
        search($root->right);
    }
    search($node);
...
<?php require("../head.php");  ?>
    ...
    <main class="container mt-4">
        <h1 class="fw-bold">Filter By: <?php echo $data["homenumber"]?></h1>
<div class="row mb-2 mt-3">

<?php
//
    foreach($searchhomenumber as $info){
        if($data["homenumber"] == $info[7]){
            // var_dump($info);
        ?>
            <div class="col-md-6">
